        ...
        <main class="pt-32 pb-24 relative overflow-hidden">
          ...
          <div class="relative max-w-4xl mx-auto px-4 sm:px-6">
            ...
            <div class="grid sm:grid-cols-3 gap-5 mb-14">
              {PREREQS.map((c) => (
                <div key={c.title} class="bg-ink-900 border border-white/10 rounded-3xl p-7 text-center">
            ...
            <div class="mb-14">
              ...
              <div class="flex flex-wrap gap-3">
                {ROLES.map((r) => (
                  <span key={r} class="px-4 py-2 rounded-xl bg-ink-800 border border-white/10 text-gray-300 text-sm font-medium hover:border-brand-500/30 transition-colors cursor-default">{r}</span>
                ))}
              </div>
            </div>
              ...
              {this.sent ? (
                ...
                <form onsubmit={(e: Event) => this.submit(e)} class="space-y-5">
                  <div class="grid sm:grid-cols-2 gap-5">
                    <Field label="نام" required bg="bg-ink-800" value={this.name} oninput={(v: string) => { this.name = v; }} />
                    <Field label="ایمیل" type="email" required ltr bg="bg-ink-800" value={this.email} oninput={(v: string) => { this.email = v; }} />
                  </div>
                  <Select
                    label="نقش مورد نظر"
                    value={this.role}

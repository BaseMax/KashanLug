      ...
      <Layout>
        <main class="pt-32 pb-24 relative overflow-hidden">
          ...
          <div class="relative max-w-2xl mx-auto px-4 sm:px-6">
            {/* Profile card */}
            <div class="relative rounded-[2.5rem] overflow-hidden border border-ui bg-card mb-8 shadow-xl shadow-black/5">
              <div class="h-36 bg-gradient-to-br from-brand-500 via-brand-600 to-brand-700 relative overflow-hidden">
              ...
              <div class="px-8 pb-8 text-center">
                <h1 class="text-2xl font-black text-fore">{PROFILE.name}</h1>
                <p class="text-brand-600 dark:text-brand-400 text-sm mt-1 font-medium" dir="ltr">{PROFILE.role}</p>
                <p class="text-muted text-sm mt-3 leading-relaxed">{PROFILE.bio}</p>

                <div class="flex justify-center flex-wrap gap-3 mt-6">
                  {PROFILE.github && (
                    <a href={PROFILE.github} target="_blank" rel="noopener"
                      class="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-ui border border-ui text-fore2 hover:text-fore hover:bg-ui2 transition-all text-sm font-medium" dir="ltr">
                  ...
                  <a href={`mailto:${PROFILE.email}`}
                    class="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-ui border border-ui text-fore2 hover:text-fore hover:bg-ui2 transition-all text-sm font-medium">
            ...
            <div class="bg-card border border-ui rounded-3xl p-7 mb-6">
              <h2 class="text-lg font-black text-fore mb-5">مهارت‌ها</h2>
              <div class="flex flex-wrap gap-2">
                {PROFILE.skills.map((skill) => (
                  <span key={skill} class="px-3 py-1.5 rounded-xl bg-brand-500/10 border border-brand-500/20 text-brand-600 dark:text-brand-400 text-sm font-medium" dir="ltr">{skill}</span>
                ))}
              </div>
            </div>

            <div class="bg-card border border-ui rounded-3xl p-7 mb-8">
              <h2 class="text-lg font-black text-fore mb-5">حوزه‌های علاقه</h2>
              <div class="flex flex-wrap gap-2">
                {PROFILE.interests.map((interest) => (
                  <span key={interest} class="px-3 py-1.5 rounded-xl bg-term-500/10 border border-term-500/20 text-term-600 dark:text-term-400 text-sm font-medium">{interest}</span>
                ))}
              </div>
            </div>
            ...
            <div class="text-center">
              <button onclick={() => window.print()}
                class="cursor-pointer inline-flex items-center gap-2 px-6 py-3 rounded-2xl font-bold text-fore2 bg-ui border border-ui hover:bg-ui2 transition-all text-sm">

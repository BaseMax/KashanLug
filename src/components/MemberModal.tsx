      ...
      >
        <div class="relative w-full max-w-md bg-ink-900 border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
          <div class="h-24 bg-gradient-to-br from-brand-700 to-brand-900 relative">
            <button
              onclick={onclose}
              class="cursor-pointer absolute top-4 left-4 w-8 h-8 rounded-xl bg-white/10 text-white flex items-center justify-center hover:bg-white/20 transition-all"
              aria-label="بستن"
            >
          ...
          <div class="flex justify-center -mt-12 mb-4 px-8">
            <img src={member.avatar} alt={member.name}
              class="w-24 h-24 rounded-2xl object-cover border-4 z-2 border-ink-900 shadow-xl" />
          </div>

          <div class="px-8 pb-8 text-center">
            <h3 class="text-xl font-black text-white">{member.name}</h3>
            <div class="text-brand-400 text-sm mt-1 mb-4">{member.role}</div>
            {member.bio && <p class="text-gray-400 text-sm leading-relaxed mb-6">{member.bio}</p>}
